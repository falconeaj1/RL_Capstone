...
class Tetris:
    ...
    def break_lines(self):
        ...
        if lines > 0:
            self.score += self.points_per_line[lines - 1] * (
                self.level + 1
            )  # lines - 1 and self.level + 1 because of 0 indexing
        return lines
    ...
    def get_properties(self, swap_piece):  # board should already be modified!
        ...
        return [
            self.get_lines(),
            self.get_holes(heights),
            self.get_bumps(heights),
            self.get_total_height(heights),
            swap_piece,
            self.queue[1].piece,
        ]
    ...
    def get_next_states(self):  # ASSUMING enough space on board and time to make moves!
        ...
        for rot in [-1, 0, 1, 2]:  # possible rotations
            for shift in range(-6, 4):  # possible shifts
                if (
                    not (self.figure.name == "O" and rot != 0)
                    and not (self.figure.name != "I" and shift == -6)
                    and not (self.figure.name in ["I", "S", "Z"] and rot in [-1, 2])
                ):
                ...
                if not (self.has_swapped):  # add the sequence with swapping
                    ...
                    if (
                        not (temp.name == "O" and rot != 0)
                        and not (temp.name != "I" and shift == -6)
                        and not (temp.name in ["I", "S", "Z"] and rot in [-1, 2])
                    ):
